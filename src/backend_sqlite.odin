...
add_time_entry :: proc(project: string, start_time: string, stop_time: string, tags: []string) -> bool {
    project_id, ok := get_or_create_project(project)
    if !ok {
        fmt.eprintln("Failed to get/create project")
        return false
...
get_entry_by_frame_id :: proc(frame_id: string, allocator := context.allocator) -> (entry: TimeEntryInfo, ok: bool) {
...
get_current_entry :: proc(allocator := context.allocator) -> (entry: TimeEntryInfo, frame_id: string, ok: bool) {
...
list_frame_ids :: proc(limit: int = 0, allocator := context.allocator) -> [dynamic]string {
    ...
    if limit > 0 {
        query = fmt.tprintf("SELECT frame_id FROM time_entries WHERE frame_id IS NOT NULL ORDER BY start_time DESC LIMIT %d", limit)
    }
    query_cstr := strings.clone_to_cstring(query, context.temp_allocator)
